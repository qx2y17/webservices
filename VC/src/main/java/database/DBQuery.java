...
public class DBQuery {
    ...
    * @see DBConnect in the database package
    ...
    public void createTableEdgeHistory() {
        ...
        //create table cispaces_article (  content varchar(255), islocked boolean )
        dbcn.updateSQL(query);//
    }
    ...
    public JSONObject getUserByUsername(String username) {
        ...
        if (rs.isEmpty()) {
            ...
            return (JSONObject) getResultListJSON(rs).get(0);
        }
    }
     ...
     * @return The newly generated user_id, or null if the user could not be
     * added
     */
    public String insertUser(String username, String generatedSecuredPasswordHash, String affiliation) {
        ...
        if (rs.isEmpty()) {
            ...
            throw new IllegalArgumentException("The selected username is already in use");
        }
    }//insertUser
    ...
    public String getLikeArticel(String keyword) {
        System.out.println("Searcj Articel for keyword: " + keyword);
        String getMetaSql = "SELECT * FROM  cispaces_article WHERE content LIKE  '%" + keyword + "%'";
        ArrayList<HashMap<String, Object>> graphMeta = dbcn.execSQL(getMetaSql);
        if (!graphMeta.isEmpty()) {
            JSONObject jsonGraph = new JSONObject();
            for(int i=0 ;i<graphMeta.size();i++){
                      jsonGraph.put("content"+i, (String) graphMeta.get(i).get("content"));
    ...
    public String insertArticle(String article){
        String[] chars=article.split(",");
        for(int i=0;i<chars.length;i++){
               String sql4 = "INSERT INTO cispaces_article (contnent) VALUES "
                    + "( '" + chars[i] + "'  )";
                ...
                if (pair.getKey().toString().equals("annot")) {
                    ...
                    if (pair.getKey().toString().equals("dtg")) {
                        ...
                        if (pair.getValue().toString().endsWith(".0")) {
                            ...
                            obj.put("dtg", pair.getValue().toString());
                        }
